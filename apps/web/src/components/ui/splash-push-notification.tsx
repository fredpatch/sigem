...
export const SplashedPushNotifications = forwardRef<
  SplashedPushNotificationsHandle,
  SplashedPushNotificationsProps
>(({ timerColor, timerBgColor }, ref) => {
  ...
  useEffect(() => {
    const setVars = (el: HTMLDivElement | null) => {
      if (!el) return;
      if (timerColor)
        el.style.setProperty("--splashed-toast-timer", timerColor);
      else el.style.removeProperty("--splashed-toast-timer");
      if (timerBgColor)
        el.style.setProperty("--splashed-toast-timer-bg", timerBgColor);
  ...
  return (
    <>
      ...
      ></div>
      <div
        ref={rtlNotificationContainerRef}
        className="notificationContainer rtlNotifications"
      ></div>
    </>
  );
});
