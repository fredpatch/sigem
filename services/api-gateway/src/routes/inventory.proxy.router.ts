...
const INVENTORY_PROXY = process.env.INVENTORY_SERVICE_URL;
const API_VERSION = "v1";
const router = Router();
const target = INVENTORY_PROXY;
...
const inventoryProxy = createProxyMiddleware({
  ...
  pathRewrite: (_path, req) => {
     ...
     * - Requête côté gateway:  /v1/assets
     * - originalUrl = "/v1/assets"
     * - On renvoie ça côté inventory-service
     */
