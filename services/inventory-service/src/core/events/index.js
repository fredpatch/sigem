...
import { setEventBus, getEventBus as _get } from "./event.bus";
import { NoOpEventBus } from "./providers/no-op-event.bus";
import { KafkaEventBus } from "./providers/kafka-event.bus";
import { env } from "src/config/env";
export async function initEvents(opts) {
    const driver = env.EVENTS_DRIVER;
    let bus;
    if (driver === "kafka") {
        console.log("[events] driver=kafka brokers=%s", process.env.KAFKA_BROKERS || "localhost:9092");
    ...
    if (opts?.eager && bus.connect) {
        try {
            ...
            console.error("⚠️ [Kafka] Eager connect failed, will retry on first emit():", e);
        }
    }
}
