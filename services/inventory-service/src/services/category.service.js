...
export class CategoryService {
    static async list(q) {
        ...
        return response({ items, total, page, limit }, null, "Categories fetched", true, 200);
    }
     ...
     */
    static async upsertSubcategory(parentId, label) {
        label = label.trim();
        ...
        if (parent.parentId !== null)
            throw new Error("Parent must be a root family");
        const sub = await Category.findOneAndUpdate({ parentId, label }, {
            $setOnInsert: {
                parentId,
                label,
    ...
    static async update(id, payload) {
        ...
        if (payload.parentId !== undefined || payload.family !== undefined) {
            ...
            const nextParentId = payload.parentId !== undefined ? payload.parentId : target.parentId;
            const nextFamily = payload.family !== undefined ? payload.family : target.family;
            if (nextParentId) {
                const parent = await Category.findById(nextParentId).select("family parentId");
                if (!parent)
                    throw new Error("Parent category not found");
                if (parent.parentId !== null)
                    throw new Error("Parent must be a root family");
